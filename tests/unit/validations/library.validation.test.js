const { libraryValidation  } = require('../../../src/validations');
const Joi = require('@hapi/joi');

describe('library validations', () => {
  describe('check - test', () => {
    it('shouldnot pass when ids not Passed', async () => {
      const { error } = Joi.compile(libraryValidation.check).validate({
        query:{   }
      });
    ...
    it('should pass when id Passed is valid', async () => {
      const { error } = Joi.compile(libraryValidation.check).validate({
        query:{  ids :'5e6a1b9e496be93758ac3e5b' }
      });
