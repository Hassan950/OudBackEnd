...
module.exports.addToRecent = async(user,body)=>{
  ...
  .updateOne({//add the new item and its type at the top of the recently searched array
      $push: {
        items: {
          $each: [body.id],
          $position: 0
...
const getPlaylist = async(id)=>{
  ...
  .populate({//get tracks details inside playlist
    path:'tracks',
    populate: {
      path: 'album artists',
      select: '-tracks -genres -released -release_date',
...
const getTrack = async(id)=>{
  ...
  .populate(//get artists, album details inside playlist
    {
      path: 'artists album',
      select: '-tracks -genres -released -release_date',
      select:'displayName images type name',
...
const getArtist = async(id)=>{
  ...
  .populate(//get genres,popularSongs details of this artist
    {
      path: 'genres popularSongs',
      select: ' name artists image album ',
      populate:{
